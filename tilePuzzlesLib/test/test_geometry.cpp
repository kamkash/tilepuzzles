...
CATCH_TEST_CASE("Geo", "[geo]") {
  ...
  CATCH_SECTION("geo vec angle") {
    ...
    float angle = GeoUtil::angleBetween(v1, v2);
    float angle1 = GeoUtil::angleBetween(v1, v3);
    math::float3 norm = GeoUtil::tcross(v1, v2);
    math::float3 norm1 = GeoUtil::tcross(v1, v3);
...
CATCH_TEST_CASE("drag angle", "[dragAngle]") {
  ...
  norm = GeoUtil::tcross(v1, v3);
  angle = GeoUtil::angleBetween(v1, v3);
  L.info("norm", norm.x, norm.y, norm.z, "angle", angle);

  float pcos1 = GeoUtil::tdot(v1,v2);
  float pcos2 = GeoUtil::tdot(v1,v3);
